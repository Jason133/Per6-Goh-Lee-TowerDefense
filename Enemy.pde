...
public class Enemy {
  ...
  private int healthConst = health ;
  ...
  public void setLevel(int level) {
    this.level = level;
    health = health + level * 50 ;
    speed = speed + level;
    // Max Health to compare against
    healthConst = health + level * 50 ;
  }
  ...
  void display() {
    ...
    map[y][x] = 0;
      pg.beginDraw();
      pg.fill(0,0,0);
      float percentage = (float)health/(float)healthConst*(float)50;
